...
# 사운드 효과 불러오기
shoot_sound = pygame.mixer.Sound(os.path.join(music_path, "shoot.mp3"))
hit_sound = pygame.mixer.Sound(os.path.join(music_path, "hit.mp3"))
game_over_sound = pygame.mixer.Sound(os.path.join(music_path, "game_over.mp3"))
mission_success_sound = pygame.mixer.Sound(os.path.join(music_path, "success.mp3"))
...
def show_start_screen():
    ...
    start_msg = start_font.render("Shoot the Apples!", True, (255, 255, 255))
    screen.blit(start_msg, (screen_width / 2 - start_msg.get_width() / 2, screen_height / 2 - start_msg.get_height() / 2))
...
def show_end_screen():
    ...
    end_msg = end_font.render(game_result, True, (255, 255, 255))
    screen.blit(end_msg, (screen_width / 2 - end_msg.get_width() / 2, screen_height / 2 - end_msg.get_height() / 2))
    score_msg = game_font.render("Your Score: {}".format(score), True, (255, 255, 255))
    screen.blit(score_msg, (screen_width / 2 - score_msg.get_width() / 2, screen_height / 2 + end_msg.get_height() / 2))
    pygame.display.update()
    pygame.time.wait(2000)
...
while running:
    ...
    # 점수 표시
    score_display = game_font.render("Score : {}".format(score), True, (0, 0, 0))
    screen.blit(score_display, (screen_width - 150, 10))
